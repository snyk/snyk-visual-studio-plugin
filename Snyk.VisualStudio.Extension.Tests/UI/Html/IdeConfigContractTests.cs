...
namespace Snyk.VisualStudio.Extension.Tests.UI.Html
{
    public class IdeConfigContractTests
    {
        ...
        [Fact]
        public void Analyze_IgnoresPerFolderKeys()
        {
            // Per-folder keys are forwarded verbatim to the LS and never checked for drift, so even
            // a brand-new folder key never counts as unmapped.
            var json = @"{
                ""folderConfigs"": [
